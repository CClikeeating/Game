...
from workV.common import load_config, read_jsonl
...
def search_segments(query: str, labels: dict[str, Any] | None = None, index_path: str | None = None, top_k: int | None = None) -> list[dict[str, Any]]:
    ...
    path = Path(index_path or config.get("default_index", "workV/outputs/indexes/segments_index.jsonl"))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Search local workV segment index.")
