...
def manifest_case_dir(batch_dir: Path, case: dict[str, Any] | str) -> Path:
    if isinstance(case, dict):
        folder = str(case.get("case_folder", "")).strip()
        if folder:
            path = Path(folder)
        ...
        if manifest_path.exists():
            ...
            for item in manifest.get("cases", []):
                if item.get("case_id") == case_id:
                    return manifest_case_dir(batch_dir, item)
    return batch_dir / "cases" / case_id
...
def prepared_block(batch_dir: Path, case_id: str, block_id: str) -> dict[str, Any]:
    manifest_path = manifest_case_dir(batch_dir, case_id) / "block_manifest.json"
...
def apply_rows(batch_dir: Path, review_path: Path) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if not case_id:
            continue
        case_path = manifest_case_dir(batch_dir, case_id) / "chat_turns.json"
...
def remove_adjacent_duplicate_turns(blocks: list[dict[str, Any]]) -> None:
    remove_adjacent_cross_block_duplicate_turns(blocks)
...
def update_manifest(batch_dir: Path, handled_failed_blocks: dict[str, set[str]]) -> None:
    ...
    for case in manifest.get("cases", []):
        ...
        case_path = manifest_case_dir(batch_dir, case) / "chat_turns.json"
        if case_path.exists():
...
def refresh_case_quality(batch_dir: Path, case: dict[str, Any]) -> dict[str, Any]:
    ...
    case_dir = manifest_case_dir(batch_dir, case)
