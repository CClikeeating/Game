...
def case_dir_from_manifest(batch_dir: Path, case: dict[str, Any]) -> Path:
    folder = str(case.get("case_folder", "")).strip()
    if folder:
        path = Path(folder)
        return path if path.is_absolute() else PROJECT_ROOT / path
    return batch_dir / "cases" / str(case["case_id"])
...
def source_image_for_block(batch_dir: Path, case_dir: Path, case_id: str, block_id: str) -> str:
    ...
    prepared_manifest = case_dir / "block_manifest.json"
...
def build_rows(batch_dir: Path) -> list[dict[str, Any]]:
    ...
    for case in manifest.get("cases", []):
        ...
        case_dir = case_dir_from_manifest(batch_dir, case)
