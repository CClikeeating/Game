...
```powershell
python -m baiou.source_pipeline.source_preparation html "data/raw/html/example.html" --run-id case_001
```

调用视觉模型识别话轮：

```powershell
python -m baiou.source_pipeline.run_pipeline --case-id case_001 --source-output case_001 --mode group
```

收集成批次包：

```powershell
python -m baiou.source_pipeline.collect_batch batch_001 case_001
```

生成或刷新人工复核表：

```powershell
python -m baiou.source_pipeline.refresh_batch_state outputs/baiou/source/batches/batch_001
```

应用人工复核：

```powershell
python -m baiou.source_pipeline.apply_human_review outputs/baiou/source/batches/batch_001 outputs/baiou/source/batches/batch_001/human_review.xlsx
```
